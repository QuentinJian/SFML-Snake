...
void Snake::IncreaseScore() {
    m_score += 10;
    m_log->Add("You add an apple. Score: " + std::to_string(m_score));
}
...
void Snake::Extend() {
    ...
    if (m_snakeBody.size() > 1) {
        ...
        if (tail_head.position.x == tail_bone.position.x) {
            ...
            if (tail_head.position.x > tail_bone.position.y - 1)
...
void Snake::Cut(int l_segments) {
    ...
    m_log->Add("You have loss a life! Lives left: " + std::to_string((m_lives)));
}
